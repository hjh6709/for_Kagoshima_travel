...
func (s *TripService) CreateTrip(ownerID string, req dto.CreateTripRequest) (dto.TripResponse, error) {
	...
	// 입력값 유효성 검증 및 기본값 보정
	// 신규 생성 요청 시 목적지 국가 정보가 누락되었거나 지원하지 않는 국가 코드일 경우, 
	// 데이터 무결성 및 하위 호환성 유지를 위해 기본 국가 코드('JP')로 보정 처리합니다.
...
func (s *TripService) UpdateTrip(id, ownerID string, req dto.UpdateTripRequest) (dto.TripResponse, error) {
	...
	if req.DestinationCountry != nil {
		...
		// 입력값 유효성 검증 및 기본값 보정
		// 잘못된 국가 코드나 빈 문자열("")이 유입되어 데이터가 오염되는 것을 방지하기 위해,
		// 지원 국가(JP, CN) 여부를 검증하고 아닐 경우 기본 국가 코드('JP')로 보정합니다.
		if dc == "" || (dc != "JP" && dc != "CN") {
			dc = "JP"
		}
