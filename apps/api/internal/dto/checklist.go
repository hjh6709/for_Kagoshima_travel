...
type ChecklistItemResponse struct {
	...
	ScheduledDate      string `json:"scheduledDate,omitempty"`      // 특정 날짜 항목의 날짜 (없으면 여행 전체)
}
...
type CreateChecklistItemRequest struct {
	...
	ScheduledDate string `json:"scheduledDate,omitempty"` // 특정 날짜에만 표시할 경우의 여행 날짜
}
...
type UpdateChecklistItemRequest struct {
	...
	ScheduledDate *string `json:"scheduledDate,omitempty"` // 특정 날짜 또는 빈 문자열(여행 전체)로 변경
}
