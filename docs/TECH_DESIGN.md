...
| 프론트엔드 | React | 컴포넌트 기반으로 여행자 화면과 여행 관리 화면을 함께 만들기 좋음 |
...
| 2차 API | REST JSON API | PWA와 여행 관리 화면에서 호출하기 단순함 |
| 2차 인증 | Go API + JWT | PWA, 여행 관리 화면, 가족 공유 페이지에서 사용하기 단순함 |
...
- 민감정보를 공개 링크와 여행 관리 화면에서 분리해야 할 때
...
- 여행 관리 화면도 React 화면으로 만들고, Go API를 호출해 데이터를 생성, 수정, 삭제한다.
...
목표:
- 로그인한 여행 관리 사용자가 여행 전후로 데이터를 직접 수정
...
인증 Authentication
  사용자가 누구인지 확인한다.
  예: 여행 관리 계정의 이메일과 비밀번호로 로그인한다.

인가 Authorization
  인증된 사용자가 어떤 권한을 가졌는지 확인한다.
  예: owner 또는 editor 권한이 있어야 일정과 장소를 수정할 수 있다.
```
...
- 여행 관리 계정은 초기에 1명 기준으로 시작하되, 이후 여행별 멤버 초대로 확장한다.
...
| 역할 | 설명 |
| --- | --- |
| `owner` | 여행을 만든 사용자. 여행 삭제, 멤버 관리, 공유 링크 관리, 전체 수정 |
| `editor` | 초대된 로그인 사용자. 일정, 장소, 루트, 여행 정보 수정 |
| `viewer` | 초대된 로그인 사용자. 로그인 후 읽기 전용 조회 |
| `public` | 로그인 없이 공유 링크로 볼 수 있는 제한된 읽기 전용 정보 |
...
| --- | --- |
| `/healthz` | 공개 |
| `/api/trips/:tripId` | owner, editor, viewer |
| `/api/trips/:tripId/schedules` | owner, editor, viewer |
| `/api/trips/:tripId/places` | owner, editor, viewer |
| `/api/trips/:tripId/routes` | owner, editor, viewer |
| `/api/trips/:tripId/share` | owner |
| `/api/share/:shareToken/**` | 공유 토큰을 가진 public 사용자 |
...
흐름:
1. owner가 공유 링크를 생성한다.
2. 링크에는 추측하기 어려운 공유 토큰이 포함된다.
3. 가족과 동행자는 로그인 없이 공유 링크로 읽기 전용 정보를 본다.
4. 공유 링크는 링크를 아는 사람이 볼 수 있는 공개 초대장이므로 수정, 삭제, 멤버 초대 권한을 주지 않는다.

공유 화면에서는 예약번호, 보험 정보, 가족 전화번호, 결제 정보 같은 민감정보를 기본 응답에서 제외한다. 운영 전에는 공유 링크 폐기 기능, 검색엔진 색인 방지, 서버 로그 접근 제한을 필수 보안 항목으로 둔다.
...
| 사용자 | 권한 |
| --- | --- |
| owner | 자신의 여행 데이터 생성, 수정, 삭제, 공유 링크 관리 |
| editor | 초대된 여행 데이터 수정 |
| viewer | 초대된 여행 데이터 읽기 전용 조회 |
| public | 공유 링크로 허용된 여행 데이터 읽기 전용 조회 |

초기에는 owner 1명 기준으로 단순하게 시작한다. 가족 공유는 읽기 전용 링크와 공개 범위 설정으로 관리하고, 로그인 사용자 공동 관리는 `trip_members` 도입 시점에 확장한다.
...
프론트에는 민감정보를 넣지 않는다. JWT secret, DB URL, 운영 계정 비밀번호 같은 값은 절대 `VITE_` 환경변수로 만들지 않는다.
...
9. 비용 여유가 생기면 PostgreSQL, 여행 관리 로그인, CRUD, 가족 공유 페이지 구현
