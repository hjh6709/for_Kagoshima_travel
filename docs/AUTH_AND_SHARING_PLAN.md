...
이 문서는 가고시마 부모님 여행 앱을 개인용 PWA에서 포트폴리오와 상용 서비스 후보로 확장하기 위한 인증, 데이터 저장, 공유 링크 전략을 정리합니다.
...
- 부모님이나 동행자는 로그인 없이 읽기 전용 링크로 본다.
- 민감정보는 공유 링크별로 노출 여부를 조절할 수 있어야 한다.
- 1차 포트폴리오 버전은 한 명의 관리자 계정과 하나의 여행 관리 흐름으로 시작한다.
...
- 관리자 API는 인증된 사용자만 접근한다.

소셜 로그인은 Phase 3에서 검토한다.

## 4. 공유 링크 방식

부모님과 가족에게는 로그인 대신 공유 링크를 제공한다.
...
| trip_id | 여행 ID |
| token_hash | 공유 토큰 해시 |
| label | 부모님용, 가족용 등 구분 이름 |
| expose_sensitive | 민감정보 노출 여부 |
| revoked_at | 폐기 시각 |
| created_at | 생성일 |
...
8. React 앱에 관리자 입력 화면을 별도 라우트로 붙인다.
9. 부모님용 화면은 공유 링크 데이터를 읽도록 전환한다.

## 8. 포트폴리오에서 강조할 포인트

- 실제 가족 문제에서 출발한 제품 문제 정의
- 부모님 사용성을 고려한 로그인 없는 공유 흐름
