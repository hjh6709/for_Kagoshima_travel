...
지도·장소 검색 변경은 테스트 mock만으로 완료 처리하지 않습니다. 배포 전에는 로컬 실제 화면에서 검색 오류·직접 입력 대안과 지도 탭 왕복을 확인하고, 운영 검증은 배포 전까지 `pending deployment`로 기록합니다. 배포 후에는 실제 사용자 계정으로 대표 장소 한 건만 검색해 핀 좌표와 Google 지도·고덕지도 길찾기 링크를 확인하여 불필요한 Places 호출을 피합니다.
